/**
 * Generates HTML email template for magic link authentication
 *
 * Creates a styled email template with sign-in button and fallback link.
 * Includes expiration notice and security information.
 *
 * @param url - Magic link URL for authentication
 * @returns HTML string for email template
 */
export function getMagicLinkEmailTemplate(url: string): string {
  return `
    <!DOCTYPE html>
    <html>
      <head>
